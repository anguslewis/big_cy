...
% Foreign real bond return is \fr{r}. For the capital-in-Foreign return r^{k*}
% do NOT write \fr{\rk} (that double-superscripts to r^{k}^{*}); write r^{k*}
% directly, e.g.  r^{k*}_{t+1}  or  \rk^{*}_{t+1} only if \rk has no built-in star.
